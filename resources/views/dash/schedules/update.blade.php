        ...
        <div class="p-5">
            <div class="flex justify-content-between items-center">
                <div class="flex items-center mr-auto">
                    <a href="{{ route('dash.schedules.index') }}"
                ...
                <div>
                    <h6 class="font-bold text-xl">Schedule</h6>
                </div>
            </div>
            <hr class="my-5">

            <form method="POST" action="{{ route('dash.schedules.update', $schedule) }}">
                @csrf
                @method('PUT')
                <!-- Modal body -->
                <div class="space-y-4 mb-6">
                    <div>
                        ...
                        <select id="subject" name="subject_id" required
                            ...
                            @foreach($subjects as $subject)
                                <option
                                    {{ $subject->id == $schedule->subject_id ? 'selected' : '' }} value="{{ $subject->id }}">{{ $subject->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select id="classroom" name="classroom_id" required
                            ...
                            @foreach($classrooms as $classroom)
                                <option
                                    {{ $classroom->id == $schedule->classroom_id ? 'selected' : '' }} value="{{ $classroom->id }}">{{ $classroom->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select id="hour" name="hour" required
                            ...
                            <option {{ $schedule->hour == '8' ? 'selected' : '' }} value="8">08:00</option>
                            <option {{ $schedule->hour == '10' ? 'selected' : '' }} value="10">10:00</option>
                            <option {{ $schedule->hour == '14' ? 'selected' : '' }} value="14">14:00</option>
                            <option {{ $schedule->hour == '16' ? 'selected' : '' }} value="16">16:00</option>
                        </select>
                    </div>
                </div>
                <!-- Modal footer -->
